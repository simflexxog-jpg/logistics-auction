...
// Debug logging to help trace OAuth issues in deployments
logger.info({ CALLBACK_URL, FRONTEND_URL, API_URL, HAS_GOOGLE_CLIENT: !!process.env.GOOGLE_CLIENT_ID }, 'OAuth route loaded with config');
...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
  ...
  router.get('/google', passport.authenticate('google', { scope: ['profile', 'email'] }));
    ...
    logger.info({ query: req.query && Object.keys(req.query).length ? req.query : null, headers: { host: req.headers.host, origin: req.headers.origin } }, 'Google callback invoked');
    const user = req.user;
